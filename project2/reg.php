...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!preg_match('/^(?=.*\d)[A-Za-z\d]{7,20}$/', $new_pwd_raw)) {
        $feedback = "❌ Password must be 7–20 characters long and contain at least one number.";
    } else {
        ...
        if (!$result) {
            $feedback = "❌ Error checking username: " . mysqli_error($conn);
        } else {
            ...
            if ($num == 0) {
                ...
                if ($success) {
                    ...
                    $feedback = "❌ Error inserting user: " . mysqli_stmt_error($insert_stmt);
                }

                // Close the prepared statement to free up resources
                mysqli_stmt_close($insert_stmt);
            } else {
                $feedback = "⚠️ Username not available.";
            }
        }
...
<body id="apply-body">
  ...
  <?php
    if (!empty($feedback)) {
      $is_success = str_starts_with($feedback, "✅");
      echo "<p class='feedback-message " . ($is_success ? "success" : "") . "'>$feedback</p>";
    }
  ?>
  ...
  <form action="reg.php" method="post" id="registration-form" class="ww-form">
    ...
    <div class="reg-row">
      ...
      <input type="text" name="user_password" id="user_password"
        ...
        pattern="^(?=.*\d)[A-Za-z\d]{7,20}$"
        title="Password must be 7–20 characters long and contain at least one number">
    </div>
